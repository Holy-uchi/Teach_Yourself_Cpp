...
* なぜその選択になるか説明せよ
* **複数の設計が考えられる場合は、それぞれの妥当性を比較せよ**

---

## 【制約】

* `std::unique_ptr`, `std::shared_ptr`, `std::weak_ptr` のみ使用可能
* 不要な共有所有は禁止
* **非所有参照は `weak_ptr` を用いる場合と用いない場合の両方を検討してよい**
...
* B↔C が非所有参照であること
* 循環参照を避けていること
* **`weak_ptr` の使用が設計にどのような影響を与えるか説明できること**
...
* **`weak_ptr` を使うために所有モデルが変わることを理解する**
